...
class Sidebar {
  ...
  setupMenuToggle() {
    ...
    menuLinks.forEach(link => {
      // Check if this link has a dropdown menu BEFORE adding listener
      const listItem = link.parentElement;
      const dropdownMenu = listItem.querySelector('.dropdown-menu');
      
      // Only add event listener if this is a dropdown toggle
      if (dropdownMenu) {
        ...
        // Add event listener only for dropdown links
        link.addEventListener('click', clickHandler);
      }
      // If no dropdown menu, don't add any listener - let the link work normally
    });
  }
